...
object UserInterpreterDoobie {
  def create[F[_] :  MonadError[?[_], UserError]](transactor: Transactor[F]): UserAlgebra[F] = new UserAlgebra[F] {
    ...
    override def save(user: User): F[Unit] =
      fr"""INSERT INTO public.user ( name, age ) VALUES ( ${user.name}, ${user.age} )""".update.run.transact(transactor).map( _ => ())
...
class UserRoutesMEOW[F[_] : Sync : Monad](userAlgebra: UserAlgebra[F])(implicit ev: HttpErrorHandler[F, UserError]) extends Http4sDsl[F] {
